...
class StatGenerator:
    ...
    def __init__(
        ...
        Args:
            ...
            attribute_names: Mapping of {attribute_id: display_name} for stats.
    ...
    def generate(
        ...
        Args:
            ...
            has_multiple_assets: Whether the item's asset group has >1 visual
                variants. If True, GetRandomAsset advances the LCG once before
                stat generation. This value comes from ITEMDEFINITIONSAUX.xxx.
        ...
        # GetRandomAsset: advance LCG for group items with >1 visual variants
        if has_multiple_assets:
            rng.advance() # TODO: Replace later with GetRandomAsset
        ...
        # Apply Multiply per-pick, then sum (the loop in CacheGeneratedData line 5242)
    ...
    def find_best_seed(
        ...
        Args:
            ...
            stat_name: One of the stat display names from attribute_names.
            has_multiple_assets: Whether the item has >1 visual assets.
        ...
        for s in range(seed_range + 1):
            result = self.generate(s, item_level, item_index, has_multiple_assets)
    ...
    def _create_attribute(self, rng: LCGStream, attr_def: dict) -> list:
        ...
        Source: Defense::Create
                SerializeFrom for int
                SerializeFrom for float
        ...
        for param in attr_def.get("p", []):
            ...
            if param_id == "value" and "i" in param:
                ...
                # min == max: no LCG advance, no stat contribution

            elif param_id in ("base", "amount", "damage") and "f" in param:
                lo_f = param["f"]["l"]
                hi_f = param["f"]["h"]
                if lo_f < hi_f:
                    rng.advance()  # consume LCG state (float stat, not tracked as base stat)

            elif param_id == "set" and "i" in param:
                lo = param["i"]["l"]
                hi = param["i"]["h"]
                if lo < hi:
                    rng.next_int_range(lo, hi)  # consume LCG state
                # min == max (always 0): no LCG advance
    ...
    def _calculate_scale_factor(
        ...
        The designLevel (dL from catalog) controls the stat curve steepness.
        At level 1, scale ~ 0.1 (minimum). At designLevel, scale approaches
        boost + base (maximum non-cap). At maxLevel with boost=0.9, scale = 1.0.
    ...
    def _apply_scale_per_pick(self, picks: list, scale_factor: float) -> dict:
        ...
        This matches the game's behavior: CalculateScaleFactor returns a float,
        then the loop in CacheGeneratedData (line 5242) calls Multiply(scaleFactor)
        on each attribute's DataInterface via vtable[3]. GetValue then sums all
        matching attributes.

        The Multiply for base stats (Defense, Strength, Health, Ability) does:
            value = max(1, int(float(value) * scaleFactor))

        Source: Defense::Data::Multiply
        """
        stats = {}
